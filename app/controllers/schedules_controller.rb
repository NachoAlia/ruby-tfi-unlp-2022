class SchedulesController < ApplicationController
  ...
  def create
    @sucursal = Sucursal.find_by_id(params[:sucursal_id])
    begin
      @schedule = @sucursal.schedules.create(schedule_params)
    rescue StandardError
      redirect_to new_sucursal_schedule_path(sucursal_id:@sucursal.id), notice:"Already a schedule created for that day"
    else
  ...
  def update
    @sucursal = Sucursal.find_by_id(params[:sucursal_id])
    @schedule = @sucursal.schedules.where(id:params[:id])

    if @schedule.update(schedule_params)
      redirect_to sucursal_path(@sucursal), notice:"Schedule was update successfully"
    else
      redirect_to edit_sucursal_schedule_path(id:params[:id],sucursal_id:@sucursal.id), notice:"Already a schedule created for that day"
    end
